...
class Siteground_Migrator_Transfer_Service {
	...
	private function before_start_transfer() {
		...
		if ( 0 !== $response['status_code'] ) {
			self::update_status(
				$response['message'],
				0,
				esc_html__( 'Please, generate another Migration Token from your SiteGround cPanel.', 'siteground-migrator' )
			);
	...
	private function check_size( $transfer_info, $wp_size ) {
		...
		if ( $wp_size > $transfer_info->free_space ) {
			self::update_status(
				esc_html__( 'There is no enough free space on you new server.', 'siteground-migrator' ),
				0,
