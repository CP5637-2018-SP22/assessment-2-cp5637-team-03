 ...
 *
 * @package team03
 * @subpackage team03 0.3@since 0.3
 */

if ( ! function_exists( 'team03_add_cat_blog_four' ) ) :
    /**
     * Add cat blog four section
     *
     * @since team03 0.3
     */
    function team03_add_cat_blog_four() {
      // Check if cat blog four is enabled on frontpage
      $cat_blog_four_enable = apply_filters( 'team03_section_status', true, 'cat_blog_four_enable' );
      ...
      $section_details = apply_filters( 'team03_filter_cat_blog_four_section_details', $section_details );
      if ( empty( $section_details ) ) {
          return;
      }

      // Render cat blog four section now.
      team03_render_cat_blog_four( $section_details );
    } 
endif;
add_action( 'team03_primary_content', 'team03_add_cat_blog_four', 70 );


if ( ! function_exists( 'team03_cat_blog_four_section_details' ) ) :
    /**
     ...
     */
    function team03_cat_blog_four_section_details( $input ) {
        ...
        $content      = array();
              ...
              if ( ! empty( $posts ) ) {
                  ...
                  foreach ( $posts as $key => $post ) {
                    ...
                    $content[$i]['date']      = get_post_meta( $post_id, 'team03-event-date' );
                    $content[$i]['time-from']      = get_post_meta( $post_id, 'team03-event-time-from', true  );
                    $content[$i]['time-to']      = get_post_meta( $post->ID, 'team03-event-time-to', true );
                    $content[$i]['location']      = get_post_meta( $post_id, 'team03-event-location', true  );
...
add_filter( 'team03_filter_cat_blog_four_section_details', 'team03_cat_blog_four_section_details' );


if ( ! function_exists( 'team03_render_cat_blog_four' ) ) :
    /**
     * Add cat blog four
     *
     * @since team03 0.3
     */
    function team03_render_cat_blog_four( $section_details ) { 
      $options = team03_get_theme_options();
            ...
            <?php foreach ( $section_details as $content ): ?>
              <div class="slider-item">
                ...
                <div class="course-contents-wrapper">
                  ...
                  <div class="buttons">
                    <a href="<?php echo esc_url( $content['url'] );?>" class="btn btn-blue"><?php esc_html_e( 'Learn More', 'team03' ); ?><i class="fa fa-angle-right"></i></a>
                  </div><!-- end .buttons -->
