 ...
 * @since team03 0.3
 */
if ( ! function_exists( 'team03_add_category_blog_two' ) ) :
  /**
   * Add Category Blog two
   *
   *@since team03 0.3
   */
  function team03_add_category_blog_two() {

    // Check if cat blog two is enabled on frontpage
    $category_blog_two_enable = apply_filters( 'team03_section_status', true, 'category_blog_two_enable' );
    ...
    $section_details = apply_filters( 'team03_filter_category_blog_two_details', $section_details );
    ...
    team03_render_category_blog_two( $section_details );
  }
endif;
add_action( 'team03_primary_content', 'team03_add_category_blog_two', 50 );


if ( ! function_exists( 'team03_get_category_blog_two_details' ) ) :
  /**
   ...
   */
  function team03_get_category_blog_two_details( $input ) {
    ...
    if ( ! empty( $args ) ) {
      ...
      foreach ( $posts as $key => $post ) {
          ...
          if ( has_post_thumbnail( $page_id ) ) {
              $img_array = wp_get_attachment_image_src( get_post_thumbnail_id( $page_id ), 'team03-category-image' );
          } else {
...
add_filter( 'team03_filter_category_blog_two_details', 'team03_get_category_blog_two_details' );


if ( ! function_exists( 'team03_render_category_blog_two' ) ) :
  /**
   ...
   *
   */
   function team03_render_category_blog_two( $content_details = array() ) {
        $options                        = team03_get_theme_options();
